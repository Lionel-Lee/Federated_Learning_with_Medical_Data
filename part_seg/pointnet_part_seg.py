...
def get_transform_K(inputs, is_training, bn_decay=None, K = 128):
...
def get_transform(point_cloud, is_training, bn_decay=None, K = 15):
    """ Transform Net, input is BxNx15 gray image
        Return:
            Transformation matrix of size 15xK """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value

    input_image = tf.expand_dims(point_cloud, -1)
    net = tf_util.conv2d(input_image, 64, [1,15], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='tconv1', bn_decay=bn_decay)
    ...
    with tf.variable_scope('transform_XYZ') as sc:
        assert(K==15)
        weights = tf.get_variable('weights', [128, 15*K], initializer=tf.constant_initializer(0.0), dtype=tf.float32)
        biases = tf.get_variable('biases', [15*K], initializer=tf.constant_initializer(0.0), dtype=tf.float32) + tf.constant(np.eye(K).flatten(), dtype=tf.float32)
        transform = tf.matmul(net, weights)
        transform = tf.nn.bias_add(transform, biases)

    #transform = tf_util.fully_connected(net, 3*K, activation_fn=None, scope='tfc3')
    transform = tf.reshape(transform, [batch_size, 15, K])
    return transform


def get_model(point_cloud, input_label, is_training, cat_num, part_num, \
		batch_size, num_point, weight_decay, bn_decay=None):
    """ ConvNet baseline, input is BxNx15 gray image """
    end_points = {}

    with tf.variable_scope('transform_net1') as sc:
        K = 15
        transform = get_transform(point_cloud, is_training, bn_decay, K = 15)
